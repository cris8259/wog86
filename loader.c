...
load_elf (FILE * f)
{
  ...
  if (ehdr.e_type == ET_DYN)
    {
      map_addr =
	mmap ((void *) addr, (len + PAGESIZE - 1) & (~(PAGESIZE - 1)),
	      PROT_NONE,
	      MAP_PRIVATE | (ehdr.e_type ==
			     ET_EXEC ? MAP_FIXED : 0) | MAP_ANONYMOUS |
	      MAP_NORESERVE, -1, 0);
	  ...
	  if (phdr.p_filesz < phdr.p_memsz)
	    {
	      map_addr =
		mmap ((void
		       *) (((addr & (~(PAGESIZE - 1)) + phdr.p_filesz +
			     PAGESIZE - 1)) & (~(PAGESIZE - 1))),
		      (phdr.p_memsz - phdr.p_filesz + PAGESIZE -
		       1) & (~(PAGESIZE - 1)), prot,
...
int
load_interp (const char *filename)
{
